...
class DenseED(nn.Module):
    def __init__(
        ...
        if len(blocks) > 1 and len(blocks) % 2 == 0:
            raise ValueError(
                "length of blocks must be an odd number, but got {}".format(len(blocks))
            )
        ...
        last_trans_up = last_decoding(
            num_features, out_channels, drop_rate=drop_rate, upsample=upsample
        )
