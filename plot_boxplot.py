...
from unet import TurbNetG, TurbNetG_noSkip_Light, TurbNetG_Light, TurbNetG_Light_small
...
#model = TurbNetG_Light(channelExponent=channelExponent)
...
q1 = np.quantile(boxplot_error, 0.25)
 
# finding the 3rd quartile
q3 = np.quantile(boxplot_error, 0.75)
med = np.median(boxplot_error)
 
# finding the iqr region
iqr = q3-q1
minBox = boxplot_error.min()
maxBox = boxplot_error.max()
 
# finding upper and lower whiskers
upper_bound = q3+(1.5*iqr)
lower_bound = q1-(1.5*iqr)

count = np.sum(boxplot_error > upper_bound)
totalSize = len(boxplot_error)
percentVal = (count*100)/totalSize
...
bp = axes.boxplot(data, patch_artist = True, showfliers=True, notch=True,widths=0.5)
