...
We normalize each quantity (darcy flow $(q_x, q_y)$ and temperature $T$) separately such that they are restricted to the $[-1, 1]$ range.
This simplifies the learning task for the NNs as has been shown previously e.g. in \citep{LeCun2012,Wiesler2011}.
...
\begin{figure}[!htb]
   ...
   \caption{Visualization of the U-net type network architecture. The encode and decoder paths feature a 2d convolutions with a kernelsize of $4$. Both paths are connected via skip connections in each level.}
   \label{fig:arch}
\end{figure}
...
The input is given as a two-channel image with $64x64$ pixel values corresponding the the darcy flow $q$.
The general shape is similar to a simple ``U-net'' \cite{Ronneberger2015} where the input is convolved into coarser and coarser features on a contracting path until a bottleneck level is reached. 
...
As an output the network produces a single channel image at the same $64x64$ resolution of the predicted temperature plume.
...
Our assumption was that features which are convolved down to a single pixel value don't hold much relevant information anymore and we can thus save some model complexity.
...
\begin{figure}[!htb]
   ...
   \begin{subfigure}{0.9\textwidth}
      \includegraphics[width=\textwidth]{img/13_comparison.png}
      \vspace{-1cm}
   \end{subfigure}
   \begin{subfigure}{0.9\textwidth}
      \includegraphics[width=\textwidth]{img/58_comparison.png}
      \vspace{-1cm}
   \end{subfigure}
   \begin{subfigure}{0.9\textwidth}
      \includegraphics[width=\textwidth]{img/96_comparison.png}
      \vspace{-1cm}
   \end{subfigure}
   \caption{Prediction and simulation groundtruth for three different samples from the test dataset. The rightmost column displays the absolute error in \degree C.}
   \label{fig:results}
\end{figure}

We trained the aforementioned model on a dataset of $n_c=278$ samples. $n_v=56$ of these where randomly selected as validation data during training.
The model was trained using the ADAM optimizer \citep{adam} with a fixed learning rate $lr=0.0004$.
After training for $n_{ep}=60000$ epochs we show a selection of three different predictions of the trained model in figure \ref{fig:results}.
These were made on a held out test set of size $n_h=120$ which was never before seen by the model.
Show in each row are the models prediction, the target temperature field from the numerical solution and the point-wise error between the two.
We can nicely observe that in all cases the general flow direction and shape of the temperature plume matches the target.
Quantifying the error we see each example having different magniutes ranging from $0.025$ \degree C to $0.5$ \degree C.
\todo{this is not so bad, rel. error over whole dataset 1\% and high errors mostly due to "small" misalignement of plume}
Furthermore the analytical LAHM model from \cite{Pophillat2020} is overlaid in the middle column.
This clearly demonstrates the improvement in quality that our new method provides.
\todo{e.g. in a real-world scenario a bending plume shifting by ~10m could make the difference when deciding where to place gwhps}
...
\todo{Web app currently uses analytical model. Want to add this as it uses non-uniform velocities.}
...
\todo{add anonymous github repository}
