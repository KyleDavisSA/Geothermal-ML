...
%If we need to optimize the locations of heat pumps throughout a city, then an initial baseline simulation can be run, and the initial velocity field of the city can be obtained. 
%Snapshots from this velocity field can be used to train the neural network, and an optimization procedure can be run to determine the thermal interference of thousands of combinations of heat pumps. 
...
%\dimline[extension start length=0, extension end length=0] {($(0,0)+(-0.21,0)$)}{($(0,2)+(-0.21,0)$)}{$L$};
...
%\draw($(1.4,0.07)$) -- node[right, black, align=right]{$\Gamma_\text{no-slip}$} ($(1.4,0.07)$);  % no-slip
%\draw($(0.85,0.31)$) -- node[right, black, align=right]{Fluid} ($(0.85,0.31)$);  % fluid
%\draw($(0.85,0.56)$) -- node[right, black, align=right]{Solid} ($(0.85,0.56)$);  % solid
%\draw($(0.85,-0.06)$) -- node[right, black, align=right]{Solid} ($(0.85,-0.06)$);  % solid
%\draw [thick,dash dot] (-0.05,0.25) -- (2.1,0.25);
